...
+
Note that makedeb reserves the extension all command names starting with *makedeb-{extension}* (where *{extension}* is the extension name). Declaring any functions with such a name in your PKGBUILD may result in naming conflicts with extensions, and is undefined behavior.
