...
main(){
    ...
    [ -z "${pkgdesc}" ] && no_desc=true || no_desc=false
    ...
    for control_field in "${MERGED_CONTROL_FIELDS[@]}"; do
        ...
        case "${arrIN[0]}" in
            ...
            "Maintainer") 
                ...
                maintainer=""
            ;;
            "Description") pkgdesc=""; no_desc=false;;
            "License") license="";;
            "Homepage") url="";;
            *)
            ;;
    ...
    if ${no_desc}; then
        warning2 "$(printf "$(gettext "'%s' must be specified." )" 'pkgdesc')"
        pkgdesc="Package generated by ${makepkg_program_name}"
    fi
