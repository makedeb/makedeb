...
usage() {
	...
	printf -- "$(gettext "  --needed              Do not automatically reinstall the built package(s) after building")\n"
...
if (( NODEPS || ( VERIFYSOURCE && !SYNCDEPS ) )); then
	...
	if (( "${SYNCDEPS}" )); then
        ...
        install_missing_dependencies "${APTARGS}" "${additional[@]}" "${predepends[@]}" "${depends[@]}" "${makedepends[@]}" "${checkdepends[@]}"
    else
